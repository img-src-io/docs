...
```rust
let res = images_api::delete_image(&config, "img_abc123").await?;
println!("Deleted: {}", res.success);
println!("Paths: {:?}", res.deleted_paths);
```
...
let res = images_api::update_visibility(&config, "img_abc123", req).await?;
println!("Visibility: {}", res.visibility);
```
...
let res = settings_api::update_settings(&config, Some(req)).await?;
println!("Updated: {}", res.message);
```
...
for preset in &res.presets {
    println!("{}: {:?}", preset.name, preset.params);
}
...
// Delete preset
let res = presets_api::delete_preset(&config, "thumbnail").await?;
println!("Deleted: {}", res.message);
```
