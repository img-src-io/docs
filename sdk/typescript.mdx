...
console.log(uploadResponse?.id);         // "a1b2c3d4e5f67890"
console.log(uploadResponse?.size);       // 102400 (bytes)
console.log(uploadResponse?.format);     // "jpeg"
console.log(uploadResponse?.isNew);      // true
console.log(uploadResponse?.visibility); // "public"
```
...
console.log(metadataResponse?.metadata?.mimeType);          // "image/jpeg"
console.log(metadataResponse?.metadata?.originalFilename);   // "photo.jpg"
console.log(metadataResponse?.metadata?.dominantColor);      // "3a7bd5" (hex without #)
console.log(metadataResponse?.metadata?.uploadedAt);         // Date object
console.log(metadataResponse?.visibility);                   // "public"
console.log(metadataResponse?.urls);                         // { original, webp, avif, jpeg, png }
```
...
console.log(`Status: ${usageResponse?.planStatus}`);         // "active" | "cancelling" | "expired"
console.log(`Images: ${usageResponse?.totalImages}`);
console.log(`Storage: ${usageResponse?.storageUsedGb} GB`);  // also storageUsedMb, storageUsedBytes

// Plan limits
const limits = usageResponse?.planLimits;
console.log(`Max uploads/month: ${limits?.maxUploadsPerMonth}`);
console.log(`Max storage: ${limits?.maxStorageBytes} bytes`);

// Current period usage
const period = usageResponse?.currentPeriod;
console.log(`Period: ${period?.period}`);                    // "2025-01"
console.log(`Uploads: ${period?.uploads}`);
console.log(`Bandwidth: ${period?.bandwidthBytes} bytes`);
console.log(`Transformations: ${period?.transformations}`);

// Remaining credits
const credits = usageResponse?.credits;
console.log(`API requests left: ${credits?.apiRequests}`);
console.log(`Transformations left: ${credits?.transformations}`);
```

## HTTP Metadata

<Tip>
  All successful responses include an `httpMeta` field containing the raw `Request` and `Response` objects.
  This is useful for debugging or accessing response headers.
</Tip>

```typescript
const result = await client.images.getImage("img_abc123");

// Access raw HTTP metadata
const { response, request } = result.httpMeta;
...
try {
  ...
  if (error instanceof errors.ErrorResponse) {
    console.log(`Code: ${error.error.code}`);       // "NOT_FOUND"
    console.log(`Message: ${error.error.message}`);  // "Image not found"
    console.log(`Status: ${error.error.status}`);    // 404
  }
  // Unstructured HTTP error (non-JSON response)
  else if (error instanceof errors.ImgsrcDefaultError) {
    console.log(`HTTP ${error.httpMeta.response.status}`);
    console.log(`Body: ${error.httpMeta.body}`);
  }
  // Response shape mismatch
  else if (error instanceof errors.ResponseValidationError) {
    console.log(`Validation: ${error.pretty()}`);
  }
  // Request was aborted
  ...
  else if (error instanceof errors.InvalidRequestError) {
  ...
  else if (error instanceof errors.UnexpectedClientError) {
...
import type {
  ...
  ListPresetsResponse as ListPresetsOpResponse,
  CreatePresetResponse,
  GetPresetResponse,
  UpdatePresetResponse,
  DeletePresetResponse as DeletePresetOpResponse,
  GetUsageResponse,
...
} from "@img-src/sdk/models/errors";
```
