...
<Tip>
  Output format is determined by the file extension. Change `.jpg` to `.webp`, `.avif` or `.jxl` to convert. <Tooltip tip="JXL has very limited browser support (~12%). Only Safari 17+ partial support. Use with fallback." cta="Check browser support" href="https://caniuse.com/jpegxl">JXL has limited browser support (?)</Tooltip>
</Tip>
